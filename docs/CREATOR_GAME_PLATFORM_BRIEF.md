...
### 1) Core OS Simulation Systems
- Browser-native desktop shell (boot screen, desktop, taskbar, start menu, context menus, windows)
- Virtual multi-drive filesystem with persistent user content
- Modular runtime for features (enable/disable/configure at runtime)
- Plugin architecture for externally packaged features
- Semantic event bus + command bus for low-coupling orchestration

### 2) App Ecosystem
- Productivity: Notepad, Calculator, Calendar, Find Files, Control Panel, Display Properties
- Creator tools: Paint, Script Runner (RetroScript IDE)
- Media: Media Player, Video Player, Winamp-style audio UX
- Communication: Inbox, Chat Room, Instant Messenger, Phone
- Games/engagement: Minesweeper, Solitaire, FreeCell, Snake, Asteroids, DOOM, SkiFree, Zork
- System apps: My Computer, Recycle Bin, Task Manager, Run Dialog, Help System

### 3) RetroScript + Event-Driven Orchestration
- Script files can launch apps, manipulate files, emit events, and present guided story beats
- Autoexec support enables startup choreography for campaigns and scripted tours
- Script + terminal workflows allow live, presenter-controlled demonstrations

### 4) Multimedia and Narrative Delivery
- Integrated sound/video playback inside the retro shell
